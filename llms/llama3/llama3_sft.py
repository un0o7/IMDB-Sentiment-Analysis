...
test_data = {
    ...
    ["Given a movie review, determine whether it is positive or negative."] *
    len(test_data),
    "input": [data["text"] for data in test_data],
    "output":
    ["positive" if data["label"] == 1 else "negative" for data in test_data]
}
...
def process_func(example):
    ...
    response = tokenizer(f"{example['output']}<|eot_id|>",
...
args = TrainingArguments(output_dir="./output/llama3",
...
peft_model_id = "LLM-Research/Meta-Llama-3-8B-Instruct-lora"
