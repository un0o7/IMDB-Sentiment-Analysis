...
class Trainer:
    ...
    def evaluate(self):
        ...
        json.dump(
            scores,
            open(f"output/{args.model_name.split('/')[-1]}_semi_scores.json",
                 'w'))
        return {
            'accuracy': accuracy,
            'precision': precision,
...
def main(args):
    ...
    unsupervised_dataset = MsDataset.load('imdb', split='unsupervised')
    unsupervised_labels = json.load(open('output/unsupervised_labels.json'))

    # 合并train和unsupervised数据
    train_dataset = [{
        'text': data['text'],
        'label': data['label']
        ...
        'text': data['text'],
        'label': label
    } for data, label in zip(unsupervised_dataset, unsupervised_labels)]
...
if __name__ == "__main__":
    ...
    metrics = main(args)
    metrics["seed"] = seed
    metrics["model_name"] = args.model_name.split("/")[-1] + '_semi'
